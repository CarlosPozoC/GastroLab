...
namespace GastroLabApp.Controllers
{
    ...
    public class UsuarioController : Controller
    {
        ...
        [HttpGet("Opiniones/{UsuarioId}")]
        [ProducesResponseType(200, Type = typeof(IEnumerable<Opinion>))]
        [ProducesResponseType(400)]
        public IActionResult GetOpinionesByReceta(int UsuarioId)
        {
            if (!usuarioRepository.UsuarioExist(UsuarioId))
                return NotFound();
            var opiniones = mapper.Map<List<OpinionDto>>(usuarioRepository.GetOpinionesByUsuario(UsuarioId));
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            return Ok(opiniones);
        }

        [HttpGet("Recetas/{UsuarioId}")]
        [ProducesResponseType(200, Type = typeof(IEnumerable<Ingrediente>))]
        [ProducesResponseType(400)]
        public IActionResult GetIngredientesByReceta(int UsuarioId)
        {
            if (!usuarioRepository.UsuarioExist(UsuarioId))
                return NotFound();
            var recetas = mapper.Map<List<RecetaDto>>(usuarioRepository.GetRecetasByUsuario(UsuarioId));
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            return Ok(recetas);
        }
