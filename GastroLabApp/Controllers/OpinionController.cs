...
namespace GastroLabApp.Controllers
{
    ...
    public class OpinionController:Controller
    {
        ...
        [HttpGet("Usuario/{OpinionId}")]
        [ProducesResponseType(200, Type = typeof(Usuario))]
        [ProducesResponseType(400)]
        public IActionResult GetUsuarioOpinion(int OpinionId)
        {
            var usuario = mapper.Map<UsuarioDto>(opinionRepository.GetUsuarioOpinion(OpinionId));
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            return Ok(usuario);
        }
